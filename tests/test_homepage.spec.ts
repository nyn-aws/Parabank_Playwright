...
const HEADING_ACCOUNTS_OVERVIEW = "Accounts Overview";
const HEADING_ACCOUNT_DETAILS = "Account Details";
const HEADING_ACCOUNT_ACTIVITY = "Account Activity";
const HEADING_TRANSFER_FUNDS = "Transfer Funds";
const HEADING_FIND_TRANSACTIONS = "Find Transactions";
const HEADING_TRANSACTION_DETAILS = "Transaction Details";
const HEADING_BILL_PAYMENT_SERVICE = "Bill Payment Service";
const HEADING_BILL_PAYMENT_COMPLETE = "Bill Payment Complete";
const HEADING_UPDATE_PROFILE = "Update Profile";
const HEADING_PROFILE_UPDATED = "Profile Updated";
const HEADING_CUSTOMER_LOOKUP = "Customer Lookup";
const HEADING_OPEN_NEW_ACCOUNT = "Open New Account";

// Success/info message constants
const MSG_ACCOUNT_CREATED = "Congratulations, your account is now open.";
const MSG_LOGIN_SUCCESS =
  "Your account was created successfully. You are now logged in.";
const MSG_LOGIN_FAILURE = "The username and password could not be verified.";
const MSG_USERNAME_EXISTS = "This username already exists.";
const MSG_PASSWORD_MISMATCH = "Passwords did not match.";
const MSG_LOGIN_INFO_FOUND = "Your login information was located successfully.";
const MSG_TRANSFER_COMPLETE = "Transfer Complete!";
const MSG_NO_TRANSACTIONS = "No transactions found.";
const MSG_SEE_ACCOUNT_ACTIVITY = "See Account Activity for more details.";

// Validation error message constants
const MSG_FIRST_NAME_REQUIRED = "First name is required.";
const MSG_LAST_NAME_REQUIRED = "Last name is required.";
const MSG_ADDRESS_REQUIRED = "Address is required.";
const MSG_CITY_REQUIRED = "City is required.";
const MSG_STATE_REQUIRED = "State is required.";
const MSG_ZIP_CODE_REQUIRED = "Zip Code is required.";
const MSG_SSN_REQUIRED = "Social Security Number is required.";
const MSG_USERNAME_REQUIRED = "Username is required.";
const MSG_PASSWORD_REQUIRED = "Password is required.";
const MSG_PASSWORD_CONFIRM_REQUIRED = "Password confirmation is required.";

// Transaction description constants
const TRANS_DESC_FUNDS_TRANSFER_SENT = "Funds Transfer Sent";
const TRANS_DESC_FUNDS_TRANSFER_RECEIVED = "Funds Transfer Received";
...
let transactionRowsLocator: Locator;
...
export async function ensureRegisteredUser(page: Page) {
  ...
  while (!registrationSuccess) {
    ...
    if (await page.getByText(MSG_USERNAME_EXISTS).isVisible()) {
      await page.goBack();
      await getRegisterLink.click();
      continue;
    }
...
test.beforeEach(async ({ page }) => {
  ...
  accountsOverviewLink = page.getByRole("link", {
    name: HEADING_ACCOUNTS_OVERVIEW,
  });
  transferFundsLink = page.getByRole("link", { name: HEADING_TRANSFER_FUNDS });
  billPayLink = page.getByRole("link", { name: "Bill Pay" });
  findTransactionsLink = page.getByRole("link", {
    name: HEADING_FIND_TRANSACTIONS,
  });
  ...
  openNewAccountHeading = page.getByRole("heading", {
    name: HEADING_OPEN_NEW_ACCOUNT,
  });
  openNewAccountInfoText = page.getByText("What type of Account would");
  openNewAccountMinimumText = page.getByText("A minimum of $");
  ...
  transactionRowsLocator = page.locator("#transactionTable tbody tr");
  accountsOverviewHeading = page.getByRole("heading", {
    name: HEADING_ACCOUNTS_OVERVIEW,
  });
  accountDetailsHeading = page.getByRole("heading", {
    name: HEADING_ACCOUNT_DETAILS,
  });
  accountActivityHeading = page.getByRole("heading", {
    name: HEADING_ACCOUNT_ACTIVITY,
  });
  transferFundsHeading = page.getByRole("heading", {
    name: HEADING_TRANSFER_FUNDS,
  });
  transferCompleteMessage = page.getByText(MSG_TRANSFER_COMPLETE);
  findTransactionsHeading = page.getByRole("heading", {
    name: HEADING_FIND_TRANSACTIONS,
  });
  transactionDetailsHeading = page.getByRole("heading", {
    name: HEADING_TRANSACTION_DETAILS,
  });
  billPaymentServiceHeading = page.getByRole("heading", {
    name: HEADING_BILL_PAYMENT_SERVICE,
  });
  billPaymentCompleteHeading = page.getByRole("heading", {
    name: HEADING_BILL_PAYMENT_COMPLETE,
  });
  updateProfileHeading = page.getByRole("heading", {
    name: HEADING_UPDATE_PROFILE,
  });
  profileUpdatedHeading = page.getByRole("heading", {
    name: HEADING_PROFILE_UPDATED,
  });
  customerLookupHeading = page.getByRole("heading", {
    name: HEADING_CUSTOMER_LOOKUP,
  });
  accountCreatedMessage = page.getByText(MSG_ACCOUNT_CREATED);
  loginSuccessMessage = page.getByText(MSG_LOGIN_SUCCESS);
  loginFailureMessage = page.getByText(MSG_LOGIN_FAILURE);
  userExistsMessage = page.getByText(MSG_USERNAME_EXISTS);
  passwordMismatchMessage = page.getByText(MSG_PASSWORD_MISMATCH);
  loginInfoFoundMessage = page.getByText(MSG_LOGIN_INFO_FOUND);
  ...
  // Validation messages
  firstNameRequiredMsg = page.getByText(MSG_FIRST_NAME_REQUIRED);
  lastNameRequiredMsg = page.getByText(MSG_LAST_NAME_REQUIRED);
  addressRequiredMsg = page.getByText(MSG_ADDRESS_REQUIRED);
  cityRequiredMsg = page.getByText(MSG_CITY_REQUIRED);
  stateRequiredMsg = page.getByText(MSG_STATE_REQUIRED);
  zipCodeRequiredMsg = page.getByText(MSG_ZIP_CODE_REQUIRED);
  ssnRequiredMsg = page.getByText(MSG_SSN_REQUIRED);
  usernameRequiredMsg = page.getByText(MSG_USERNAME_REQUIRED);
  passwordRequiredMsg = page.getByText(MSG_PASSWORD_REQUIRED);
  confirmPasswordRequiredMsg = page.getByText(MSG_PASSWORD_CONFIRM_REQUIRED);
});
...
test.describe("Parabank - Tests Requiring User Registration", () => {
  ...
  test("SCENARIO 7: Accounts Overview Page Elements should be visible", async ({
    ...
    await expect(accountIdLocator).toHaveText(secondAccountNumber);
    await expect(accountTypeLocator).toHaveText("SAVINGS");
    await expect(accountBalanceLocator).toHaveText(
      `$${minDepositAmount.toFixed(2)}`
    );
    await expect(availableBalanceLocator).toHaveText(
      `$${minDepositAmount.toFixed(2)}`
    );
    ...
    await expect(descriptionCell).toHaveText(
      TRANS_DESC_FUNDS_TRANSFER_RECEIVED
    );
    ...
    await fromAccountSelect.selectOption(secondAccountNumber ?? "");
    await expect(openNewAccountButton).toBeVisible();
    await openNewAccountButton.click();
    await expect(
  ...
  test("SCENARIO 8: User Registration with Existing Username", async ({
    ...
    await expect(userExistsMessage).toBeVisible();
  });
  ...
  test("SCENARIO 10: User Login with Incorrect Password", async ({ page }) => {
    ...
    await expect(loginFailureMessage).toBeVisible();
  });
  ...
  test("SCENARIO 11: Clicking on 'Forgot login info?' link should navigate to the correct page", async ({
    ...
    await expect(loginInfoFoundMessage).toBeVisible();
    await expect(
  ...
  test("SCENARIO 12: Checking Default account and its balance", async ({
    ...
    await expect(accountDetailsHeading).toBeVisible();
    await expect(accountIdLocator).toHaveText(defaultAccountNumber as string);
    await expect(accountTypeLocator).toBeVisible();
    await expect(accountBalanceLocator).toBeVisible();
    await expect(availableBalanceLocator).toBeVisible();
    await expect(accountActivityHeading).toBeVisible();
    await expect(activityMonth).toBeVisible();
    await expect(activityType).toBeVisible();
    await expect(goButton).toBeVisible();
    await goButton.click();
    const noTransactions = page.getByText(MSG_NO_TRANSACTIONS);
    ...
    */
    ...
    await expect(accountIdLocator).toHaveText(newAccountNumber as string);
    await expect(accountTypeLocator).toHaveText("SAVINGS");
    await expect(accountBalanceLocator).toHaveText(
      `$${minDepositAmount.toFixed(2)}`
    );
    ...
    await expect(transferCompleteMessage).toBeVisible();
  ...
  test("SCENARIO 15: Find Transactions by Various Criteria", async ({
    ...
    await transferFundsLink.click();
    const transferAmount = (Math.floor(Math.random() * 50) + 1).toFixed(2);
    await fromAccountSelect.selectOption(firstAccountNumber?.trim() || "");
    await toAccountSelect.selectOption(secondAccountNumber?.trim() || "");
    await amountInput.fill(transferAmount);
    await transferButton.click();
    ...
    await expect(transactionTableLocator).toBeVisible();
    await expect(transactionRowsLocator).toHaveCount(
      await transactionRowsLocator.count()
    ); // Accept any number of rows
    // Verify that at least one transaction has the correct details
    const dateMatch = transactionRowsLocator.filter({
      hasText: transactionDate ?? "",
    });
    ...
    // Verify that at least one transaction has the correct details
    const dateRangeMatch = transactionRowsLocator.filter({
      hasText: transactionDate ?? "",
    });
    ...
    const amountMatch = transactionRowsLocator.filter({
  ...
  test("SCENARIO 16: Update Contact Info with Valid and Invalid Data", async ({
    ...
    await expect(firstNameRequiredMsg).toBeVisible();
    await expect(lastNameRequiredMsg).toBeVisible();
    await expect(addressRequiredMsg).toBeVisible();
    await expect(cityRequiredMsg).toBeVisible();
    await expect(stateRequiredMsg).toBeVisible();
    await expect(zipCodeRequiredMsg).toBeVisible();
  });
  ...
  test("SCENARIO 18: Complete End-to-End ParaBank Workflow", async ({
    ...
    test.setTimeout(120000); // Increase timeout to 120 seconds for this comprehensive test
    if (!registeredUser) throw new Error("registeredUser not set");
...
test.describe("Parabank User Registration with Invalid Data", () => {
  ...
  test("SCENARIO 20: User Registration with Missing Required Fields", async ({
    ...
    await expect(stateRequiredMsg).toBeVisible();
    await expect(zipCodeRequiredMsg).toBeVisible();
    await expect(ssnRequiredMsg).toBeVisible();
    await expect(usernameRequiredMsg).toBeVisible();
    await expect(passwordRequiredMsg).toBeVisible();
    await expect(confirmPasswordRequiredMsg).toBeVisible();
